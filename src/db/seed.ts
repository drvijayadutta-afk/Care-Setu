...
async function seedPatients(coordinatorId: string) {
  ...
  for (const lab of labsP1) {
    await prisma.labResult.create({ data: { patientId: p1.id, source: "manual", ...lab } });
  }
  ...
  for (const v of vitalsP1) {
    ...
    await prisma.vitalSign.create({ data: { patientId: p1.id, source: "manual", bmi, ...v } });
  }
  ...
  for (const lab of labsP2) {
    await prisma.labResult.create({ data: { patientId: p2.id, source: "manual", ...lab } });
  }
  ...
  for (const v of vitalsP2) {
    const bmi = parseFloat((v.weight / (v.height / 100) ** 2).toFixed(1));
    await prisma.vitalSign.create({ data: { patientId: p2.id, source: "manual", bmi, ...v } });
  }
  ...
  for (const lab of labsP3) {
    await prisma.labResult.create({ data: { patientId: p3.id, source: "manual", ...lab } });
  }
  ...
  for (const v of vitalsP3) {
    const bmi = parseFloat((v.weight / (v.height / 100) ** 2).toFixed(1));
    await prisma.vitalSign.create({ data: { patientId: p3.id, source: "manual", bmi, ...v } });
  }
