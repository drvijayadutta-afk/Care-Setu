...
interface ThresholdRule {
  ...
  check: (params: {
    ...
    score: number;
    cycleDay: number;
    recentCheckins: { score: number; symptoms: string[]; createdAt: Date }[];
  }) => boolean;
  severity: "info" | "caution" | "urgent";
  patientMessage: (lang: string) => string;
...
const THRESHOLD_RULES: ThresholdRule[] = [
  {
    ...
    check: ({ symptoms, cycleDay }) =>
      cycleDay >= 5 && cycleDay <= 14 && symptoms.includes("fever") && symptoms.includes("chills"),
    severity: "urgent",
    patientMessage: (lang) =>
      lang === "hi"
  ...
  },
  {
    ...
    check: ({ symptoms, recentCheckins }) => {
      if (!symptoms.includes("vomiting")) return false;
    ...
    caregiverMessage:
      "Patient has reported vomiting for 3 consecutive days. Please monitor hydration. If they cannot keep water down, take them to the hospital today.",
    notifyCoordinator: true,
    notifyDoctor: false,
  },
  {
    ...
    caregiverMessage:
      "Patient has scored 1-2 for 5 consecutive days. This pattern suggests significant distress or poor symptom control. Please check in with them and consider contacting the care team.",
    notifyCoordinator: true,
    notifyDoctor: true,
  },
  {
    ...
    check: ({ symptoms, recentCheckins }) => {
      if (!symptoms.includes("mouth_sores")) return false;
    ...
    caregiverMessage:
      "Patient has reported mouth sores for 5+ days. Please ensure they're eating and staying hydrated. This should be mentioned at the next visit.",
  ...
  },
  {
    ...
    caregiverMessage:
      "⚠️ URGENT: Patient reported breathing difficulty. Please check on them immediately. If severe, take them to hospital or call an ambulance now.",
    notifyCoordinator: true,
    notifyDoctor: true,
  },
  {
    ...
    check: ({ score, symptoms }) =>
      score === 1 && (symptoms.includes("fever") || symptoms.includes("vomiting")),
    severity: "caution",
    patientMessage: (lang) =>
      lang === "hi"
    ...
    caregiverMessage:
      "Patient scored 1 today and is reporting fever or vomiting. Please check on them.",
    notifyCoordinator: false,
    notifyDoctor: false,
  },
  {
    ...
    check: ({ recentCheckins }) => {
      if (recentCheckins.length === 0) return true;
    ...
    caregiverMessage:
      "Patient hasn't responded to check-ins for 3 days. You may want to check in with them.",
    notifyCoordinator: false,
    notifyDoctor: false,
  },
  {
    ...
    check: ({ symptoms, recentCheckins }) => {
      if (!symptoms.includes("pain")) return false;
    ...
    caregiverMessage:
      "Patient has reported pain for 5 consecutive days. Please note this for the upcoming doctor visit.",
    notifyCoordinator: false,
    notifyDoctor: true,
  },
  {
    ...
    check: ({ symptoms, score, recentCheckins }) => {
      if (!symptoms.includes("appetite_loss") || score > 2) return false;
    ...
    caregiverMessage:
      "Patient has had appetite loss for 4+ days with a low score. Please try to encourage small meals. Mention to the care team.",
    notifyCoordinator: true,
    notifyDoctor: false,
  },
  {
    ...
    caregiverMessage:
      "Patient's score dropped sharply from 4-5 yesterday to 1 today. Please check in with them.",
    notifyCoordinator: false,
    notifyDoctor: false,
  },
];
