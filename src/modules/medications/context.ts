...
const MEDICATION_CONTEXTS: Record<
  string,
  { purpose: string; timing: string; sideEffects: string[]; watchFor: string[] }
> = {
  ondansetron: {
...
export async function sendMedicationContext(patientId: string, medicationId: string) {
  const [patient, medication] = await Promise.all([
    prisma.patient.findUnique({ where: { id: patientId } }),
    prisma.medication.findUnique({ where: { id: medicationId } }),
  ]);

  if (!patient || !medication || medication.contextSent) return;

  const genericName = medication.genericName?.toLowerCase() || medication.name.toLowerCase();
  const preLoaded = Object.entries(MEDICATION_CONTEXTS).find(([key]) =>
...
export async function sendMedicationReminder(patientId: string) {
...
export async function handleMedicationResponse(patient: Patient, replyId: string) {
...
export async function handleMedicationQuery(patient: Patient, text: string) {
  const medications = await prisma.medication.findMany({
