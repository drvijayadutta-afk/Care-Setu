...
export const INTENT_CLASSIFIER_PROMPT = `You are an intent classifier for a WhatsApp-based cancer care support system in India.
...
export const EMOTIONAL_SUPPORT_DAYTIME_PROMPT = (
  patientName: string,
  language: string,
  cycleDay: number,
...
) => `You are a compassionate care companion for a cancer patient in India.
...
export const EMOTIONAL_SUPPORT_NIGHT_PROMPT = (
  patientName: string,
  language: string,
  cycleDay: number
) => `You are a quiet, gentle presence for a cancer patient who has reached out at night.
...
export const CHECKIN_EXTRACTOR_PROMPT = `Extract health information from this patient message (Indian cancer patient, post-chemotherapy).
Return JSON with:
...
export const PRE_VISIT_BRIEF_PROMPT = (
  patientName: string,
  cancerType: string,
  treatmentProtocol: string,
  currentCycle: number,
...
) => `Generate a pre-visit doctor brief for a cancer patient.
...
export const WEEKLY_SIGNAL_PROMPT = (
  patientName: string,
  cancerType: string,
  cycle: number,
  checkinData: any[],
  upcomingVisit: string | null
) => `Analyze this cancer patient's past 7 days and determine if there is a signal worth sending to their oncologist.
...
export const CAREGIVER_BRIEF_PROMPT = (
  patientName: string,
  ...
  dayProfile: any
) => `Write a morning WhatsApp message for a cancer patient's caregiver.
