...
async function sendDoctorWeeklySignals() {
    const doctors = await client_1.prisma.doctor.findMany({ where: { isOptedIn: true } });
    for (const doctor of doctors) {
        await sendSignalForDoctor(doctor.id, doctor.whatsappNumber, doctor.name);
    }
}
async function sendSignalForDoctor(doctorId, whatsappNumber, doctorName) {
